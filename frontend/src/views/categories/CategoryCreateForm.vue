...
const loadParentOptions = async () => {
  try {
    ...
    parentTreeOptions.value = formatTreeForSelect(tree)
  } catch (error) {
    console.error('Error loading parent options:', error)
  }
}

const formatTreeForSelect = (nodes) => {
  return nodes.map(node => ({
    key: node.id,
    label: node.name,
    children: node.children?.length ? formatTreeForSelect(node.children) : undefined,
  }))
}
