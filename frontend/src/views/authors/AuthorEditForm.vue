<template>
  <form @submit.prevent="handleSubmit" class="author-edit-form">
    <div class="grid">
      ...
      <div class="col-12">
        <div class="field">
          ...
          <div v-if="author.photo_url" class="mb-3">
            <img :src="author.photo_url" alt="Current photo" class="current-author-photo" />
            <div class="mt-2">
              <Button 
...
onMounted(() => {
  ...
  form.first_name = props.author.first_name
  form.last_name = props.author.last_name
  form.biography = props.author.biography || ''
  form.nationality = props.author.nationality || ''
  form.birth_date = props.author.birth_date ? new Date(props.author.birth_date) : null
  form.death_date = props.author.death_date ? new Date(props.author.death_date) : null
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (deletePhotoFlag.value) {
      formData.append('delete_photo', '1')
    }

    await authorsStore.updateAuthor(props.author.id, formData)
    ...
    if (error.response?.data?.errors) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Erreur',
        detail: error.response?.data?.message || 'Erreur lors de la mise à jour de l\'auteur',
        life: 3000
      })
    }
  } finally {
